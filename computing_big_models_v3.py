...
print('##############################')
print('this is my first model with resnet50 and 2 hidden layers (corrected the trainable weights for good results)')
print('##############################')


# ResNet50 Simple Model
base_model_resnet50 = applications.ResNet50(weights='imagenet', include_top=False, input_shape=input_shape)
base_model_resnet50.trainable = False
preprocess_input_resnet50 = applications.resnet50.preprocess_input

# ResNet50 Complex Model
complex_model_resnet50 = models.Sequential([
    layers.Input(shape=input_shape),
    layers.Lambda(preprocess_input_resnet50),
    base_model_resnet50,
...
complex_model_resnet50.compile(
    optimizer='adam',
    loss=tf.keras.losses.SparseCategoricalCrossentropy(from_logits=True),
    metrics=['accuracy'],
)

print("Training Complex Model with ResNet50:")
complex_history_resnet50 = complex_model_resnet50.fit(
    train_ds,
...
for images, labels in test_ds:
    ...
    # Make predictions
    preds = complex_model_resnet50.predict(images)
...
print('##############################')
print('this is my first model with resnet50 and 3 hidden layers (corrected the trainable weights for good results)')
print('##############################')



# ResNet50 Complex Model
more_complex_model_resnet50 = models.Sequential([
    layers.Input(shape=input_shape),
    layers.Lambda(preprocess_input_resnet50),
    base_model_resnet50,
    layers.Conv2D(16, 3, padding='same', activation='relu'),
...
more_complex_model_resnet50.compile(
    optimizer='adam',
    loss=tf.keras.losses.SparseCategoricalCrossentropy(from_logits=True),
    metrics=['accuracy'],
)
...
print("Training Complex Model with ResNet50:")
more_complex_history_resnet50 = more_complex_model_resnet50.fit(
    train_ds,
    validation_data=val_ds,
    epochs=epochs,
    verbose=1
...
for images, labels in test_ds:
    ...
    # Make predictions
    preds = more_complex_model_resnet50.predict(images)
    pred_probs = tf.nn.softmax(preds, axis=-1)
