...
print('##############################')
print('this is my first model with vgg16 and 2 hidden layers (corrected the trainable weights for good results)')
print('##############################')

base_model_vgg16 = applications.VGG16(weights='imagenet', include_top=False, input_shape=input_shape)
base_model_vgg16.trainable = False
preprocess_input_vgg16 = applications.vgg16.preprocess_input




# VGG16 Complex Model
complex_model_vgg16 = models.Sequential([
    layers.Input(shape=input_shape),
    layers.Lambda(preprocess_input_vgg16),
    base_model_vgg16,
...
complex_model_vgg16.compile(
    optimizer='adam',
    loss=tf.keras.losses.SparseCategoricalCrossentropy(from_logits=True),
    metrics=['accuracy'],
)

print("Training Complex Model with VGG16 and 2 hidden layers:")
complex_history_vgg16 = complex_model_vgg16.fit(
    train_ds,
...
for images, labels in test_ds:
    ...
    # Make predictions
    preds = complex_model_vgg16.predict(images)
...
print('##############################')
print('this is my first model with vgg16 and 3 hidden layers (corrected the trainable weights for good results)')
print('##############################')

# VGG16 Complex Model
more_complex_model_vgg16 = models.Sequential([
    layers.Input(shape=input_shape),
    layers.Lambda(preprocess_input_vgg16),
    base_model_vgg16,
    layers.Conv2D(16, 3, padding='same', activation='relu'),
...
more_complex_model_vgg16.compile(
    optimizer='adam',
    loss=tf.keras.losses.SparseCategoricalCrossentropy(from_logits=True),
    metrics=['accuracy'],
)

print("Training a more Complex Model with VGG16:")
more_complex_history_vgg16 = more_complex_model_vgg16.fit(
    train_ds,
    validation_data=val_ds,
    epochs=epochs,
    verbose=1
...
for images, labels in test_ds:
    ...
    # Make predictions
    preds = more_complex_model_vgg16.predict(images)
    pred_probs = tf.nn.softmax(preds, axis=-1)
